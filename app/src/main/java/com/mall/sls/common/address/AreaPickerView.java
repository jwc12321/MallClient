...
public class AreaPickerView extends Dialog {
    ...
    private String citySeleCode = "-1";
    private String areaSeleCode = "-1";
    ...
    public AreaPickerView(@NonNull Context context, int themeResId, List<ProvinceBean> addressBeans) {
        super(context, themeResId);
        this.addressBeans = addressBeans;
        this.context = context;
        rooView = LayoutInflater.from(context).inflate(R.layout.dialog_area_pickerview, null);
...
//设置参数
    ...
    private void initConfig() {
        ...
        params.height= ConvertDpAndPx.Dp2Px(context,500);
...
//        window.setWindowAnimations(R.style.PickerAnim);
        ...
        provinceAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                provinceCode= addressBeans.get(position).getCode();
                ...
                if (position != oldProvinceSelected) {
                    if (oldCitySelected != -1) {
                        addressBeans.get(oldProvinceSelected).getCityBeans().get(oldCitySelected).setStatus(false);
                    }
                    if (oldAreaSelected != -1) {
                        addressBeans.get(oldProvinceSelected).getCityBeans().get(oldCitySelected).getAreaBeans().get(oldAreaSelected).setStatus(false);
                    }
                    oldCitySelected = -1;
                    oldAreaSelected = -1;
                }
                cityBeans.addAll(addressBeans.get(position).getCityBeans());
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
                areaAdapter.notifyDataSetChanged();
                strings.set(0, addressBeans.get(position).getName());
                if (strings.size() == 1) {
                    strings.add("请选择");
        ...
        cityAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                citySeleCode=cityBeans.get(position).getCode();
                if (oldCitySelected != -1 && oldCitySelected != citySelected) {
                    addressBeans.get(oldProvinceSelected).getCityBeans().get(oldCitySelected).setStatus(false);
                }
                if (position != oldCitySelected) {
                    if (oldAreaSelected != -1 && cityBeans.get(position).getAreaBeans() != null) {
                        addressBeans.get(oldProvinceSelected).getCityBeans().get(oldCitySelected).getAreaBeans().get(oldAreaSelected).setStatus(false);
                    }
                    oldAreaSelected = -1;
                }
                oldCitySelected = citySelected;
                if (cityBeans.get(position).getAreaBeans() != null) {
                    areaBeans.addAll(cityBeans.get(position).getAreaBeans());
                    cityAdapter.notifyDataSetChanged();
                    areaAdapter.notifyDataSetChanged();
                    strings.set(1, cityBeans.get(position).getName());
                    if (strings.size() == 2) {
                        strings.add("请选择");
                    ...
                    areaPickerViewCallback.callCode(provinceCode, citySeleCode);
                }
            }
        });
        ...
        areaAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                strings.set(2, areaBeans.get(position).getName());
                tabLayout.setupWithViewPager(viewPager);
                viewPagerAdapter.notifyDataSetChanged();
                areaBeans.get(position).setStatus(true);
                areaSelected = position;
                areaSeleCode=areaBeans.get(position).getCode();
                ...
                areaPickerViewCallback.callCode(provinceCode, citySeleCode, areaSeleCode);
            }
        });
    ...
    public void setSelect(int... value) {
        ...
        if (value == null) {
            ...
            if (isCreate) {
                ...
                if (citySelected != -1)
                    addressBeans.get(provinceSelected).getCityBeans().get(citySelected).setStatus(false);
                cityBeans.clear();
                areaBeans.clear();
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
        ...
        if (value.length == 3) {
            strings.add(addressBeans.get(value[0]).getName());
            strings.add(addressBeans.get(value[0]).getCityBeans().get(value[1]).getName());
            strings.add(addressBeans.get(value[0]).getCityBeans().get(value[1]).getAreaBeans().get(value[2]).getName());
            tabLayout.setupWithViewPager(viewPager);
            viewPagerAdapter.notifyDataSetChanged();
            ...
            addressBeans.get(value[0]).getCityBeans().get(value[1]).setStatus(true);
            addressBeans.get(value[0]).getCityBeans().get(value[1]).getAreaBeans().get(value[2]).setStatus(true);
            cityBeans.clear();
            cityBeans.addAll(addressBeans.get(value[0]).getCityBeans());
            areaBeans.clear();
            areaBeans.addAll(addressBeans.get(value[0]).getCityBeans().get(value[1]).getAreaBeans());
            provinceAdapter.notifyDataSetChanged();
            cityAdapter.notifyDataSetChanged();
            areaAdapter.notifyDataSetChanged();
